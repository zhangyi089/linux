...
static void ext4_iomap_wb_submit_zeroed_eof(struct inode *inode,
					    loff_t range_start, int sync_mode)
{
	struct address_space *mapping = inode->i_mapping;
	loff_t pstart, plen;
	...
	/*
	 * Keep the caller's sync mode to avoid stalling the background
	 * flusher.
	 */
				 ...
				 struct writeback_control *wbc)
{
	 ...
	 */
	ext4_iomap_wb_submit_zeroed_eof(inode, range_start, wbc->sync_mode);
	...
	/*
	 * The disksize-grow-pending state may have been set while this
	 * writeback pass was running.  If so, submit the pending range
	 * so ioends submitted by this pass do not wait for the next
	 * writeback pass.
	 */
	ext4_iomap_wb_submit_zeroed_eof(inode, range_start, wbc->sync_mode);
...
static int ext4_iomap_mark_disksize_pending(struct inode *inode, loff_t from)
{
	 ...
	 *
	 * Checking i_disksize here (after folio_wait_writeback()) is safe.
	 * If a delalloc writeback of this folio was in-flight,
	 * folio_wait_writeback() has waited for its completion and the
	 * ioend has advanced i_disksize accordingly. If writeback had not
	 * started, any future writeback will carry the pagecache content
	 * that now includes the zeroed data, so no stale data can appear
	 * on disk even without the pending tracking.
	 */
